...
Это связано с тем, что chrome сразу же запускает несколько разных процессов, причём больашя часть из них не доступна для завершения из пользовательского процесса.​
Но все равно главные процессы остаются достиупны пользователю (тут это 2 из 16) и в резульатте их удаления впоследствии удаляются и все остальные (Причём, как видно из логов, 1 и 3 процессы, которыми я инициализировал запуск хрома, удалились сразу, а потом и 2-й). 
